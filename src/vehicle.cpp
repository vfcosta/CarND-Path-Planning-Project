...
void Vehicle::update_state(vector<Vehicle> vehicles)
{
  ...
  for (auto const &possible_state : possible_states)
  {
    ...
    // realize the transition to properly calculate costs
    if (possible_state == "PLCL")
      vehicle.state = "LCL";
    if (possible_state == "PLCR")
      vehicle.state = "LCR";
...
{
  if (this->lane > 2 || this->lane < 0)
    return 1;
  return 0;
}

/**
* Returns the speed cost based on the max speed permitted and the target speed for the current lane
*/
...
/**
* Calculate the safety cost based on collisions that may occurs based on the current state and
* other vehicle predictions
*/
double Vehicle::safety_cost(vector<vector<double>> trajectory, vector<Vehicle> vehicles)
{
  if (state.compare("KL") == 0)
    return 0; // consider that collision will always be avoided when state is KL
  double cost = 0;
  for (auto &vehicle : vehicles)
  {
    ...
    if (collider.collision)
    {
      cout << "HIT: " << state << "**********************************************************" << endl;
      cout << "C1] "
           << " lane: " << this->lane << " s: " << this->s << endl;
      cout << "C2] "
           << " lane: " << vehicle.lane << " s: " << vehicle.s << endl;
      cost = 1;
      break;
    }
    if (cost == 1)
...
/**
* Return possible states transitions based on the current state
*/
vector<string> Vehicle::get_possible_successor_states()
{
  vector<string> states;
  states.push_back("KL");
  if (this->state == "LCL" || this->state == "PLCL")
    states.push_back("LCL");
  if (this->state == "LCR" || this->state == "PLCR")
    states.push_back("LCR");
  if (this->state == "PLCL" || this->state == "KL")
    states.push_back("PLCL");
  if (this->state == "PLCR" || this->state == "KL")
...
/**
* Calculates target speed and acceleration for a lane based on other vehicles
*/
